...
storage.setItem('f',datacontent);
    var $m = storage.getItem('m');
    var $f = storage.getItem('f');
    var isEmbed = false, F = { "DISSHARE": "" };
    var RULE = { FIELDSRULE: [] };
    var ADVPERM = {};
    var M = { "FRMNM": "协会宝会议活动表单", "DESC": "", "LANG": "cn", "LBLAL": "L", "CFMTYP": "T", "CFMMSG": "提交成功。", "SDMAIL": "0", "CAPTCHA": "1", "IPLMT": "0", "SCHACT": "0", "INSTR": "0", "ISPUB": "1", "GID": "", "HEIGHT": 552 }
    M = JSON.parse($m);
    // console.log(M)
    $('#title').text(M.FRMNM);
    $('#desc').text(M.DESC);
    var F = [{ "LBL": "姓名", "TYP": "name", "REQD": "0", "UNIQ": "0", "SCU": "pub", "FMT": "short", "DEF": "", "INSTR": "", "CSS": "" }, { "LBL": "手机", "TYP": "phone", "REQD": "0", "UNIQ": "0", "SCU": "pub", "FMT": "mobile", "INSTR": "", "CSS": "" }, { "LBL": "地址", "TYP": "address", "REQD": "0", "SCU": "pub", "INSTR": "", "CSS": "", "SUBFLDS": { "ZIP": {}, "PRV": {}, "CITY": {}, "DTL": {} } }, { "LBL": "邮箱", "TYP": "email", "FLDSZ": "m", "REQD": "0", "UNIQ": "0", "SCU": "pub", "DEF": "", "INSTR": "", "CSS": "" }, { "LBL": "所属岗位", "TYP": "dropdown", "FLDSZ": "m", "REQD": "0", "SCU": "pub", "INSTR": "", "CSS": "", "ITMS": [{ "VAL": "会长", "CHKED": "0" }, { "VAL": "理事", "CHKED": "0" }, { "VAL": "会员", "CHKED": "0" }] }];
...
DEFFLD = {
    ...
    number: {
        html: '<label class="desc"><span class="req hide"> *</span></label><div class="content"><input type="text" data-rule-regex="^[0-9]*$" data-msg-regex="请输入数字" maxlength="32" class="input fld" /></div>'
    },
    textarea: {
    ...
    phone_mobile_cn: '<input type="text" data-rule-phone="true" maxlength="32" class="s input" /> <button type="button" class="btn sendcode hide">发送验证码</button>',
    phone_mobile_en: '<input type="text" data-rule-phone="true" maxlength="32" class="s input" /> <button type="button" class="btn sendcode hide">发送验证码</button>',
    url: {
    ...
    email: {
        html: '<label class="desc"><span class="req hide"> *</span></label><div class="content"><input type="text" data-rule-regex="^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+" data-msg-regex="请输正确的邮箱" maxlength="64" class="m input" /></div>'
    },
    name: {
...
function createField(r, q, num) {
                ...
                if (q.TYP === 'name' && q.FMT) {
                    ...
                    if (q.TYP === 'address') {
                        ...
                        if (q.TYP === 'radio') {
                            ...
                            m.find('input').attr('name', 'F' + num);
                            for(let s= 0;s< m.find('input').length;s++){
                                m.find('input').eq(s).attr('value', m.find('label').eq(s).html());
                            }
                            m.find('input').eq(0).prop('checked', true)
                        } else {
                            if (q.TYP === 'checkbox') {
                                m.empty();
                                var b;
                                ...
                                $.each(q.ITMS, function (j, c) {
                                    ...
                                    b.find('input').attr('name', 'F' + num);
                                    for(let s= 0;s< b.find('input').length;s++){
                                        b.find('input').eq(s).attr('value', b.find('label').eq(s).html());
                                    }
    ...
    if (q.INSTR) {
        r.append(DEFFLD.icon).append(l).find('label.desc').eq(0).append(`<p style="color:#999;font-weight:400;margin:0;">${q.INSTR}</p>`)
    }
...
function initValidate() {
    ...
    d = function () {
        ...
        for (i = 0; i < g.length; i++) {
            ...
            E = t.attr('name'),
            s;
            //当为必填时，根据控件类型进行判断是否填写 by Wilson
            if (o === '1') {
                var C = true;
                if (n === 'radio') {
    ...
    $('#btnSubmit,#btnSave,#btnActSave').bind({
        click: function () {
            var laerr = $('label.error')
            if(laerr.length !=0 ){
                var arrs = []
                for(let s= 0;s<laerr.length;s++){
                    if(laerr[s].style.display !='none'){
                        arrs.push(s)
                    }
                }
                if(arrs.length!=0){
